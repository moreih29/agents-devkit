...
// assets/hooks/prompt-router/handler.ts
...
function parseInvocationCall(call) {
...
function applyTemplate(templateStr, args, templateDef) {
...
function expandInvocationExpression(expression, harness, invocations) {
...
function expandInvocations(input, harness, invocations) {
  let result = "";
  let i = 0;
  ...
  for (const tag of detected) {
    switch (tag.name) {
      ...
      case "d":
        if (!hasPlan) {
          decision = "block";
          block_reason = `[d] tag requires an active plan session. ${expand('{{skill_activation skill="nx-plan"}}', harness)} first.`;
        } else {
