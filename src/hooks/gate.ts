...
IMPORTANT: Always backup before modifying. Never delete without user approval.`,
    ...
    if (match.primitive === 'consult') {
      ...
      if (sid) {
        ...
        writeFileSync(statePath(sid, 'consult'), JSON.stringify({ active: true, phase: 'explore', startedAt: new Date().toISOString() }));
      }
      respond({
        continue: true,
...
PHASE TRACKING: Update phase as you progress: nx_state_write({ key: "consult", value: { active: true, phase: "<current_phase>" } }). Clear when done: nx_state_clear({ key: "consult" }).`,
    ...
    if (match.primitive === 'plan') {
      ...
      if (sid) {
        ensureDir(sessionDir(sid));
        writeFileSync(statePath(sid, 'plan'), JSON.stringify({ active: true, phase: 'analyze', startedAt: new Date().toISOString() }));
      }
      respond({
        continue: true,
...
Before finishing, call nx_state_clear({ key: "parallel" }) to deactivate.`,
  ...
  // Auto Mode: 키워드/라우팅 매칭 없을 때 자동으로 auto 활성화
