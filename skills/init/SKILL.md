...
Onboard Lattice into an existing project — scan, triage, and restructure project knowledge for optimal context efficiency.

## Trigger
- User says: "init", "온보딩", "lattice 설정", "프로젝트 초기화"
- Explicit tag: `[init]`
- Direct invocation: `/lattice:init`

## What It Does

Scans the existing project, triages documentation, and generates structured Lattice knowledge files — replacing scattered .md files with a clean, efficient knowledge base.
...
- Lattice plugin must be installed and active
- `.claude/lattice/knowledge/` directory should NOT already have project-specific files (if it does, ask user if they want to re-init)
...
| **Redundant** | Info that Lattice handles better (workflow instructions, agent tips) | → Skip |
...
- "How to use Claude" instructions → Redundant (Lattice replaces this)
  ...
  questions: [
    {
      ...
      options: [
        ...
        {
          label: "유지 + 추가만",
          description: "기존 CLAUDE.md 건드리지 않고 Lattice 연동만 추가"
        },
        {
          label: "완전 교체",
          description: "CLAUDE.md를 Lattice 전용으로 교체 (기존은 백업)"
        }
      ]
    },
    {
...
```
cp CLAUDE.md .claude/lattice/knowledge/original-claude-md.md
```

2. **Generate knowledge files** in `.claude/lattice/knowledge/`:
```
architecture.md:
...
## Lattice
- 프로젝트 지식: `.claude/lattice/knowledge/` 참조
- 브랜치 계획: `.claude/lattice/plans/` 참조
- 원본 CLAUDE.md: `.claude/lattice/knowledge/original-claude-md.md`
```

4. **Add `.claude/lattice/` to git** if not already tracked.

### Phase 5: VERIFY (자동)

Quick validation:
1. All generated knowledge files are valid markdown
2. `lat_knowledge_read` can read them successfully
...
- If `.claude/lattice/knowledge/` already has files, ask before overwriting
...
If lattice knowledge already exists, `[init]` should:
1. Detect existing knowledge files
2. Ask: "기존 knowledge가 있습니다. 다시 초기화할까요?"
3. If yes: backup existing knowledge to `.claude/lattice/knowledge/backup-{date}/`
4. Re-run full workflow

## Gate Integration

Gate detects `[init]` tag and "온보딩" keyword → injects init skill context.
Init does NOT use sustain/pipeline — it's an interactive one-shot process like consult.
