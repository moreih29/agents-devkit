...
Agent({ subagent_type: "claude-nexus:engineer", name: "engineer-1", team_name: "<project>",
  prompt: "태스크 T1 구현. nx_task_update(in_progress) 착수, 완료 후 nx_task_update(completed) 호출. codebase/ knowledge를 즉시 갱신하고 Lead에게 SendMessage 보고. 기술 문제는 architect에게 에스컬레이션. 브리핑: {briefing}" })
...
- Do/Check 에이전트 개별 shutdown (How agents는 세션 수명 — 유지)
- 사용자에게 결과 보고
...
3. **Do agents = 실행** — Lead가 결정. Engineer는 codebase/ 즉시 갱신, Researcher는 reference/ 즉시 기록.
...
// 2. Lead↔How agent 토론 후 태스크 확정, Do agent 합류
...
// 4. 사이클 완료 후 Do/Check 퇴장 (How agents는 세션 수명)
