...
- **Load tasks.json**:
  ...
  - **Absent** → auto-invoke `Skill({ command: "nx-auto-plan" })` to generate tasks.json. Do NOT ask the user — `[run]` implies execution intent.

### Step 2: Execute

#### Task Registration

For each task, call `TaskCreate({ subject: "<label>" }) then nx_task_update({ taskId, status: "pending" })` to register progress tracking. Keep registrations to a maximum of 10. If tasks exceed 10, group related tasks by a natural grouping unit such as `plan_issue` or target file, so that registered entries stay within 10.
...
- At the same moment, update progress tracking with `TaskCreate({ subject: "<label>" }) then nx_task_update({ taskId, status: "in_progress" })` / `TaskCreate({ subject: "<label>" }) then nx_task_update({ taskId, status: "completed" })`. Reuse the exact label set at initial registration.
