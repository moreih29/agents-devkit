...
요구사항이 불명확할 때 구조화된 상담으로 정리한다. 선택지를 제시하고 방향을 합의하는 것이 전부다. 실행은 하지 않으며, 사용자가 `[plan]`으로 전환하여 실행을 시작한다.
...
4. **실행 안 함** — 상담 후 사용자가 `[plan]`으로 전환을 결정
