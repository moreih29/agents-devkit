...
**[research] Lead 판단: 도구 0회, 요청 텍스트만으로 직감 추정. 4+ 서브태스크 또는 복수 도메인/소스 필요 시 Team Path. 판단 결과를 근거와 함께 1줄로 표시 (예: "sub-path로 진행합니다 — 조사 항목 2개, 단일 도메인").**

### Phase 1: Analyze (Lead 직접)

- 사용자에게 모드 고지 + 판단 근거: "[research] sub-path로 진행합니다 — {판단 근거}"
...
- 사용자에게 모드 고지 + 판단 근거: "[research] team-path로 진행합니다 — {판단 근거}"
