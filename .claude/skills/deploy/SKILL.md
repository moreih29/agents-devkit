...
### 1-1. Core knowledge drift 검사

`/claude-nexus:nx-sync` 스킬을 호출하여 `.nexus/core/` 문서와 소스 코드 간 불일치를 탐지 + 수정한다.

- 수정사항 있으면: `git add` → `git commit -m "docs: sync core knowledge"`

### 1-2. CLAUDE.md Nexus 섹션 검증

빌드가 `templates/nexus-section.md`와 프로젝트 `CLAUDE.md` 마커 내부를 자동 동기화하므로, 별도 drift 체크 불필요.

`bun run dev` 또는 `node esbuild.config.mjs` 실행으로 최신 상태가 보장된다.

불일치가 발견되면: `bun run dev` 재실행 후 `git add CLAUDE.md && git commit -m "docs: sync CLAUDE.md"`

### 1-3. 스킬 spec 하드코딩 검사

`skills/*/SKILL.md` + `.nexus/core/**/*.md` 내용에서 구명칭 잔존 검사.

방법:
1. `agents/*.md`에서 현재 에이전트 이름 목록 추출
2. `gate.ts` EXPLICIT_TAGS에서 현재 태그 목록 추출
3. 스킬/core 파일에서 이 목록에 없는 에이전트/태그/스킬 이름 grep
