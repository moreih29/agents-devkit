...
## 목표
omc와 omo를 심층 분석하고, Lattice 오케스트레이션 시스템의 구체적이고 자세한 설계를 완성한다.

## 완료 조건
- [x] omc 심층 분석 문서 10개 작성
- [x] omo 심층 분석 문서 10개 작성
- [x] 두 시스템 심층 비판적 분석 (`01-deep-critique.md`)
- [x] Lattice 시스템 설계 제안서 (`02-new-system-design.md`)
...
## 현재 상태
설계 문서 작성 및 비판적 검토가 완료되었고, Lattice 자체 규칙에 따른 knowledge/plans 구조를 초기화했다.
...
## 다음 브랜치 예정
`feature/lattice-phase1` — Lattice MVP 구현:
1. `.claude-plugin/` 매니페스트
2. `hooks.json` (Gate 모듈)
3. MCP 서버 (Core 도구)
4. 에이전트 5개 (Steward, Artisan, Scout, Compass, Sentinel)
5. 스킬 1개 (Sustain)

### 전환 전략
- omc를 글로벌에서 삭제하지 않음
- 프로젝트 `.claude/settings.json`에서 omc 비활성화 + Lattice 활성화
