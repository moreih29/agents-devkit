...
```typescript
lat_lsp_hover({ file, line, character })
→ textDocument/hover → 타입 정보 반환

lat_lsp_goto_definition({ file, line, character })
→ textDocument/definition → 정의 위치 반환

lat_lsp_find_references({ file, line, character })
→ textDocument/references → 참조 목록 반환

lat_lsp_diagnostics({ file })
...
```typescript
lat_ast_search({ pattern, language?, path? })
...
- E2E: Lattice 자체 TypeScript 소스로 테스트
...
## 참조
- `.claude/lattice/knowledge/mcp-tools.md` — 도구 설계
- `.claude/lattice/knowledge/architecture.md` — 시스템 아키텍처
