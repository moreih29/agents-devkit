...
### 수정
1. **`handleSessionEnd()`**: 현재 세션 디렉토리의 워크플로우 상태 파일(sustain, pipeline, parallel) 삭제
...
저장 위치: `.lattice/tasks/` (gitignore, 프로젝트 로컬)

```typescript
// .lattice/tasks/{id}.json
interface Task {
...
### MCP 도구 (4개)
- `lat_task_create({ title, description?, tags? })` → 태스크 생성
- `lat_task_list({ status?, tags? })` → 필터링 목록
- `lat_task_update({ id, status?, title?, description?, tags? })` → 상태/내용 변경
- `lat_task_summary()` → 요약 (todo/in_progress/done/blocked 카운트 + 진행 중 목록)
...
5. (선택) 바로 실행으로 전환 (cruise/pipeline 연계)
