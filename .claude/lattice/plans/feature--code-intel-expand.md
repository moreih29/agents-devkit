...
- [x] AST replace 도구 추가
- [x] Python/Rust/Go LSP 검증 (lattice-test에서 테스트 프로젝트)
- [x] E2E 테스트 확장
- [x] 빌드 + 캐시 동기화
...
## Unit 3: AST replace
`ast.ts`에 `lat_ast_replace` 추가:
```typescript
lat_ast_replace({ pattern, replacement, language?, path?, dryRun? })
→ 매칭 → 치환 → 변경된 파일 목록
```
dryRun=true 시 치환 없이 매칭만 반환.

## Unit 4: 다언어 검증
`~/workspaces/projects/lattice-test/`에 미니 프로젝트 생성:
