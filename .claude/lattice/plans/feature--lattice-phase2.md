# Plan: feature/lattice-phase2

## 목표
Lattice 워크플로우 시스템 완성 — Parallel + Pipeline 프리미티브 구현 및 에이전트 확장으로 복합 워크플로우(cruise 등) 조합이 가능한 상태 달성.

## 설계 원칙
Claude 자체가 오케스트레이터. 별도 런타임 없이, **스킬 프롬프트 + 상태 파일 + 컨텍스트 주입(Pulse)**으로 워크플로우를 구동.
- Sustain: Gate가 Stop 차단 → Claude가 계속 작업
...
- [x] 에이전트 4개 추가 (Strategist, Lens, Analyst, Tinker)
- [x] 복합 워크플로우 스킬 1개 (cruise = Pipeline + Sustain + Parallel)
- [x] E2E 테스트 확장 (28개, 전부 통과)
- [x] lattice-test에서 수동 검증
- [x] 빌드 + 캐시 동기화
...
```typescript
// .lattice/state/sessions/{id}/parallel.json
...
3. `lat_state_write`로 Parallel 상태 활성화 (tasks 배열 포함)
4. `Agent` 도구로 에이전트 병렬 호출 (한 메시지에 여러 Agent 호출)
5. 결과 수집 후 `lat_state_write`로 태스크 상태 업데이트
6. 전체 완료 시 `lat_state_clear({ key: "parallel" })`
...
```typescript
// .lattice/state/sessions/{id}/pipeline.json
...
2. `lat_state_write`로 Pipeline 상태 활성화 (stages 배열 포함)
3. 각 단계마다: 적절한 에이전트 호출 → 결과를 state에 기록 → 다음 단계 진행
4. 단계 내에서 Sustain/Parallel 중첩 가능
5. 전체 완료 시 `lat_state_clear({ key: "pipeline" })`
...
| `agents/lens.md` | Lens | 코드 리뷰 | high | full | opus |
| `agents/analyst.md` | Analyst | 심층 분석/리서치 | high | full | opus |
| `agents/tinker.md` | Tinker | 디버거 | medium | standard | sonnet |

각 에이전트는 Phase 1과 동일한 포맷: frontmatter + Role/Guidelines.
Pipeline/Parallel 스킬에서 에이전트를 참조할 수 있도록 역할이 명확해야 함.

### Unit 5: 복합 워크플로우 스킬 (cruise)
**범위**: Pipeline + Sustain + Parallel 조합 스킬.

파일: `skills/cruise/SKILL.md`
...
1. Pipeline 상태 활성화
2. 각 단계마다 Sustain 활성화 → 에이전트 호출 → 완료 후 Sustain 해제 → 다음 단계
3. implement 단계는 Parallel로 태스크 분배
4. 전체 완료 후 Pipeline + Sustain 모두 해제

### Unit 6: E2E 테스트 확장 + 빌드
**범위**: Phase 2 기능 테스트 + 빌드 파이프라인.

- `test/e2e.sh` 확장: Parallel/Pipeline 상태 CRUD, Gate 차단 시나리오
- `bun run build` → `bun run dev` → lattice-test 수동 검증
- 수동 검증: cruise 스킬 실행 → 단계별 진행 확인

## 구현 순서 요약

```
Unit 1 (Parallel) → Unit 2 (Pipeline) → Unit 3 (Pulse) → Unit 4 (에이전트) → Unit 5 (cruise) → Unit 6 (테스트)
```
...
### 상태 스키마 확장 vs 새 도구
기존 `lat_state_write/read/clear`로 충분. Parallel/Pipeline 전용 도구는 만들지 않음.
...
## 참조
- `.claude/lattice/plans/feature--lattice-phase1.md` — Phase 1 계획 (완료)
- `.claude/lattice/knowledge/workflows.md` — 3 프리미티브 설계
- `.claude/lattice/knowledge/agents-catalog.md` — 에이전트 카탈로그
