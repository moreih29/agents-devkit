...
|------|------|------|---------|---------------|
| **Steward** | 오케스트레이터 | high | full | "집사/관리인" — 전체를 관리하되 직접 일하지 않음 |
| **Artisan** | 코드 구현 | medium | standard | "장인" — 품질에 대한 자부심을 내포 |
| **Scout** | 코드 탐색 | low | minimal | "정찰병" — 빠르게 코드베이스를 탐색 |
| **Compass** | 아키텍처 설계 | high | full | "나침반" — 방향을 제시. READ-ONLY |
| **Sentinel** | 검증/보안 | medium | standard | "보초" — 결과 검증 + 보안 감시. mode로 구분 |
| **Strategist** | 계획 수립 | high | full | "전략가" — 전략적 계획 수립 |
| **Lens** | 코드 리뷰 | high | full | "렌즈" — 코드를 확대하여 세밀하게 관찰 |
| **Analyst** | 심층 분석 | high | full | "분석가" — 이미 직관적이고 보편적인 이름 |
| **Tinker** | 디버거 | medium | standard | "수선공" — 시행착오를 통한 수정 |
| **Critic** | 비평가 | high | full | "비평가" — 품질을 엄격하게 평가 |
| **Weaver** | 테스트 엔지니어 | medium | standard | "직조공" — 테스트 안전망을 짜는 행위 |
| **Scribe** | 문서 작성 | low | minimal | "서기관" — 기록의 중요성 강조 |
...
1. **verifier + security-reviewer → Sentinel**: 모두 "검사"가 본질. 프롬프트 내 mode 파라미터로 구분.
2. **qa-tester 제거**: Weaver가 QA 역할 포함.
3. **document-specialist 제거**: Scribe + Scout 조합으로 커버.
