...
```
→ 캐시 위치: `~/.claude/plugins/cache/lattice/claude-lattice/0.1.0/`
...
### 4. 테스트 폴더 생성
```bash
mkdir -p ~/workspaces/projects/lattice-test/.claude
cat > ~/workspaces/projects/lattice-test/.claude/settings.local.json << 'EOF'
{
  "enabledPlugins": {
...
```
src/ 수정 → bun run dev → lattice-test에서 Claude Code 재시작 → 검증
```
