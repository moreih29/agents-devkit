# Plan: feature/nexus-phase1

## 목표
Nexus MVP — 플러그인이 설치·인식되고 핵심 기능이 동작하는 최소 구현.

## 완료 조건
- [x] 플러그인 매니페스트 (`plugin.json`, `.mcp.json`)
- [x] 프로젝트 스캐폴딩 (`package.json`, `tsconfig.json`, esbuild 설정)
- [x] Gate 훅 (Stop: Nonstop 차단, UserPromptSubmit: 키워드 감지)
- [x] MCP 서버 Core 도구 8개 (state 3 + knowledge 2 + memo 2 + context 1)
- [x] 에이전트 5개 (Lead, Builder, Finder, Architect, Guard)
- [x] Nonstop 스킬 1개
...
파일:
- `package.json` — name: claude-nexus, type: module, files 목록
...
- `src/mcp/tools/state.ts` — nx_state_read, nx_state_write, nx_state_clear
- `src/mcp/tools/knowledge.ts` — nx_knowledge_read, nx_knowledge_write
- `src/mcp/tools/memo.ts` — nx_memo_read, nx_memo_write
- `src/mcp/tools/context.ts` — nx_context
- `src/shared/paths.ts` — 경로 유틸 (.nexus/, .claude/nexus/)
...
### Unit 3: Gate 훅
**범위**: Stop 이벤트에서 Nonstop 차단 + UserPromptSubmit에서 키워드 감지.
...
동작:
- Stop: `.nexus/state/sessions/{id}/nonstop.json` 존재 + active → block + iteration 증가
...
검증: E2E 자동 테스트 + nexus-test에서 수동 Nonstop 워크플로우 검증 완료

### Unit 4: 에이전트 5개
**범위**: Phase 1 에이전트 마크다운 정의.

파일:
- `agents/lead.md` — 오케스트레이터 (high/full)
- `agents/builder.md` — 코드 구현 (medium/standard)
- `agents/finder.md` — 코드 탐색 (low/minimal)
- `agents/architect.md` — 아키텍처 설계 (high/full, READ-ONLY)
- `agents/guard.md` — 검증/보안 (medium/standard)

포맷: frontmatter (name, tier, context, disallowedTools, tags) + Role/Guidelines 본문

검증: Claude Code에서 `Agent(subagent_type="nexus:builder")` 호출 가능

### Unit 5: Nonstop 스킬
**범위**: 지속 실행 워크플로우 스킬.

파일:
- `skills/nonstop/SKILL.md` — 스킬 정의 (프롬프트)

동작:
- MCP nx_state_write로 Nonstop 상태 활성화
- Gate 훅이 Stop 차단
- 완료 시 nx_state_clear로 비활성화

검증: `/nexus:nonstop` 호출 → Stop 차단 → cancel로 해제
...
- `~/workspaces/projects/nexus-test/`에서 omc 비활성화 + Nexus 검증
- E2E 자동: `bash test/e2e.sh` (21케이스)
- E2E 수동: Finder 에이전트 호출, nx_context MCP 호출, Nonstop 워크플로우 (키워드→차단→해제)

## 기술 결정

### omc와의 차이 (단일 CJS 스크립트)
omc: `node run.cjs scripts/target.mjs` (이중 스폰)
Nexus: `node "$CLAUDE_PLUGIN_ROOT"/scripts/gate.cjs` (단일 CJS, esbuild 번들)

### MCP 서버 이름
`lat` — 짧고 명확. 도구는 `nx_state_read` 등으로 접근.
...
3. **additionalContext는 제안**: 중요 동작(Nonstop 활성화)은 훅이 직접 상태 파일 생성해야 함
...
## 참조
- `.claude/nexus/knowledge/` — 전체 설계 문서
- `.claude/nexus/knowledge/dev-workflow.md` — 로컬 개발/테스트 가이드
