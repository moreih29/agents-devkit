...
- [ ] consult 스킬에 Intent-First 반영 (SKILL.md 수정, 코드 변경 없음)
...
**달성 목표**:
- [consult]/[do]/[d] 3개 태그 체계 동작
- 단일 실행 스킬이 목표에 따라 역할을 동적으로 구성
- consult 스킬의 "실행 안 함 → 적절한 태그 추천" 재검토 ([do]밖에 없으면 추천이 무의미)
...
**달성 목표**:
- Lead가 에이전트 위임을 기본으로 동작 (최소 Director 스폰)
- 에이전트 스폰 시 역할별 briefing이 시스템에 의해 자동 구성
- Director 의도 검증 + QA 산출물 검증의 2단계 검증 동작
...
**달성 목표**:
- 에이전트 반복 실패 시 자동 감지 + 에스컬레이션 체인 동작
- task_close 시 교훈이 memory/에 자동 추출
- memory가 다음 세션의 에이전트 briefing에 반영되어 자기 발전 메커니즘 동작
