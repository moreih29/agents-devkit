...
| 진입점 | 소스 | 빌드 산출물 | 역할 |
|--------|------|-------------|------|
| MCP Server | `src/mcp/server.ts` | `bridge/mcp-server.cjs` | 도구 제공 (knowledge, rules, task, decision, consult, branch, LSP, AST) |
...
│   └── tools/             ← 도구별 모듈 (markdown-store, context, task, decision, artifact, consult, branch, lsp, ast)
├── shared/
