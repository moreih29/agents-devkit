# ADR: 부트스트래핑 전략 (omc → Nexus 전환)

## 상태
확정 (2026-03-19)

## 맥락
Nexus는 Claude Code 오케스트레이션 플러그인이다. 개발 도구 자체를 개발해야 하므로, "무엇으로 개발하는가"가 중요하다. 현재 omc가 설치되어 있다.

## 결정
3단계 부트스트래핑으로 전환한다.

### Stage 0: omc로 Nexus MVP 개발
- omc의 autopilot/ralph/ultrawork 활용하여 Nexus Phase 1 구현
- 별도 테스트 프로젝트에서 Nexus 플러그인 설치/검증
- **완료 조건**: Phase 1의 5가지 (매니페스트, Gate 훅, MCP 서버, 에이전트 5개, Nonstop 스킬)가 동작

### Stage 1: 프로젝트 레벨에서 omc 비활성화 + Nexus 활성화
- **omc를 삭제하지 않는다.** 프로젝트 `.claude/settings.json`에서 비활성화:
```json
{ "plugins": { "omc": { "enabled": false } } }
```
- Nexus 플러그인을 프로젝트 레벨로 설치
- `.claude/nexus/knowledge/`에 Nexus 프로젝트 자체의 knowledge 기록
- **장점**: 다른 프로젝트에서는 omc가 그대로 동작. 전환이 양방향으로 가능.
- **확인 필요**: Claude Code가 프로젝트 레벨에서 글로벌 플러그인의 훅을 완전히 비활성화하는지 (Phase 1 구현 시 실제 테스트)

### Stage 2: Nexus로 Nexus 개발 (Dogfooding)
- Phase 2, Phase 3를 Nexus 자체로 개발
