...
4. 적응형 라우팅 → 요청 카테고리 분류(10개) → 에이전트 위임 지시 (`Agent()` 호출 구문 포함)
   - 에이전트 직접 언급 시 override (한글 조사 필수: "Finder로", 또는 대문자 시작, 또는 `nexus:` 접두사)
   - 히스토리 기반: 동일 카테고리에서 2회 이상 같은 에이전트 선택 시 자동 추천
   - 카테고리: 버그수정(debugger), 리뷰(reviewer), 테스트(tester), 리팩토링(builder), 탐색(finder), 설계(architect), 계획(strategist), 분석(analyst), 문서(writer), 대규모구현(Lead 위임 제안)
...
Task / Context / Constraints / Expected Output / Resources / Decision Points.
...
### 위임 강제 (모드 인식)
활성 워크플로우 모드에 따라 에이전트 위임 강제:
- `consult` 모드: Strategist/Analyst 위임 권장
- `plan` 모드: Strategist→Architect→Reviewer 합의 루프 강제
- 모드 없음: 적응형 라우팅(Gate 감지 결과) 적용
