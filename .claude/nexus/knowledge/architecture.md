...
| 진입점 | 소스 | 빌드 산출물 | 역할 |
|--------|------|-------------|------|
| MCP Server | `src/mcp/server.ts` | `bridge/mcp-server.cjs` | 도구 제공 (knowledge, task, decision, consult, LSP, AST) |
...
```
src/
├── hooks/gate.ts          ← 유일한 훅 모듈 (3개 이벤트 + CLAUDE.md 동기화 + consult cleanup)
├── mcp/
│   ├── server.ts          ← McpServer 인스턴스 + 도구 등록
│   └── tools/             ← 도구별 모듈 (knowledge, context, task, decision, artifact, consult, lsp, ast)
├── shared/
