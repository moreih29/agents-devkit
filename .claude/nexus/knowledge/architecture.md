...
- Builder(구현), Finder(탐색), Architect(설계), Guard(검증) 등

### 4. 점진적 복잡성
- 5가지 워크플로우 스킬: consult, plan, init, setup, sync
...
### context 수준별 메시지 필터링
```
[minimal] safety + workflow 메시지만 주입
[standard] + guidance 메시지 추가
[full] 모든 우선순위 메시지 주입 (info 포함)
```
...
| Finder | 코드 탐색 | low (haiku) | minimal |
| Architect | 아키텍처 설계 | high (opus) | full |
| Guard | 검증/보안 | medium (sonnet) | standard |

### Phase 2 에이전트
| 이름 | 역할 | tier | context |
|------|------|------|---------|
| Strategist | 계획 수립 | high (opus) | full |
| Reviewer | 코드 리뷰 | high (opus) | full |
...
| 이름 | 역할 | tier | context |
|------|------|------|---------|
| Tester | 테스트 엔지니어 | medium (sonnet) | standard |
| Writer | 문서 작성 | low (haiku) | minimal |

## 훅 모듈 (3 스크립트 + MCP)

| 모듈 | 실행 방식 | 담당 이벤트 | 역할 |
|------|-----------|-------------|------|
| Gate | hooks.json (별도 프로세스) | Stop, UserPromptSubmit | Stop 차단 (활성 모드/에이전트 체크), 키워드 감지 (consult/plan/init/setup/[d]) |
| Pulse | hooks.json (별도 프로세스) | PreToolUse | 컨텍스트 주입 (Whisper 패턴 + 워크플로우 상태 + 에이전트별 수준 분기), Guard 내장, 위임 강제 |
| Tracker | hooks.json (별도 프로세스) | SubagentStart/Stop, Session | 에이전트/세션 추적, 위임 규칙 주입 (SessionStart) |
| Memory | MCP 도구 (nx_* 호출 시) | — | knowledge, state CRUD (훅이 아닌 MCP 도구 그룹) |
...
| 스킬 | 유형 | 설명 |
|------|------|------|
| Consult | 대화형 | 적응형 깊이 + 차원 추적 + 실행 브릿지. 8단계 워크플로우 |
| Plan | 대화형 | 합의 루프(Strategist→Architect→Reviewer) + 적응형 형식성(소/중/대) |
