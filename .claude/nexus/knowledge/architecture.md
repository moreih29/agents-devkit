...
### 4. 점진적 복잡성
- 5가지 워크플로우 스킬: consult, dev, init, setup, sync
...
> Analyst → Director, Builder+Debugger → Engineer, Guard → QA로 통합.

## 훅 모듈 (1 스크립트 + MCP)

| 모듈 | 실행 방식 | 담당 이벤트 | 역할 |
|------|-----------|-------------|------|
| Gate | hooks.json (별도 프로세스) | Stop, PreToolUse, UserPromptSubmit | tasks.json pending 체크 → Stop 차단, team 모드 Agent 직접 호출 차단, 키워드 감지 (consult/dev/[d]) |
