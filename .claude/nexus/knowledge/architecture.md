...
| 모듈 | 실행 방식 | 담당 이벤트 | 역할 |
|------|-----------|-------------|------|
| Gate | hooks.json (별도 프로세스) | Stop, UserPromptSubmit | tasks.json pending 체크 → Stop 차단, 키워드 감지 (consult/plan/init/setup/[d]) |
| Memory | MCP 도구 (nx_* 호출 시) | — | knowledge, task, decision CRUD (훅이 아닌 MCP 도구 그룹) |

## MCP 도구 (Core + Code Intel)

### Core
`nx_knowledge_read/write`, `nx_memo_read/write`, `nx_context`, `nx_task_list/add/update`, `nx_decision_add`, `nx_plan_archive`
...
| Plan | 대화형 | Lead 직접 초안, tasks.json 중심, 소규모(서브에이전트)/대규모(Teams) |
