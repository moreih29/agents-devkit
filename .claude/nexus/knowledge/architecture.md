...
### 4. 점진적 복잡성
- 5가지 워크플로우 스킬: consult, team, init, setup, sync
...
| 모듈 | 실행 방식 | 담당 이벤트 | 역할 |
|------|-----------|-------------|------|
| Gate | hooks.json (별도 프로세스) | Stop, UserPromptSubmit | tasks.json pending 체크 → Stop 차단, 키워드 감지 (consult/team/init/setup/[d]) |
...
| Team | 대화형 | Lead 직접 초안, tasks.json 중심, 항상 Agent Teams 활용 |
