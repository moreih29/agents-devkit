...
| `reopen` | issue_id | decided → discussing으로 되돌림. decisions.json에서 `consult === issue_id`인 항목 제거 |
...
- `nx_consult_status`: `d.consult === issue.id` 기반으로 결정 항목 join
- `nx_consult_update reopen`: `d.consult === issue_id`인 항목 decisions.json에서 제거
