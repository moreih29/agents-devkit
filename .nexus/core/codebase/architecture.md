...
|-------------|--------|--------------|------|
| MCP Server | `src/mcp/server.ts` | `bridge/mcp-server.cjs` | Tool provision (core, rules, context, task, meet, artifact, briefing, LSP, AST) |
...
│   └── tools/             ← per-tool modules (core-store, markdown-store, context, task, meet, artifact, branch, lsp, ast)
├── shared/
...
- `skills/*/SKILL.md` — skill definitions (5 skills: nx-run, nx-meet, nx-init, nx-setup, nx-sync)
...
| `.nexus/state/` | gitignore | Runtime state (tasks.json, meet.json, agent-tracker.json, artifacts/) |
...
- **[meet] session persistence**: On [meet] tag, continues existing session if meet.json is present. gate.ts checks meet.json existence to branch between resume/start guidance.
- **Unified archive**: nx_task_close archives meet+tasks to `.nexus/history.json` (project-level). cycle includes branch field and meet key. Auto-migrates legacy per-branch history.json. Deletes source files (meet.json, tasks.json).
...
- **Tag-based guidance via additionalContext**: Tags ([meet], [d], [run], [rule]) detected in UserPromptSubmit and routed to handlers. Pipeline enforcement is handled by the tasks.json PreToolUse block.
...
- **TeamCreate mandatory for meet/run**: [meet] tag forces TeamCreate for How agent spawning. [run] mode enforces team_name via gate PreToolUse block.
- **meet.json decisions inline**: Decisions are stored directly inside MeetIssue.decision field — no separate decisions.json. nx_task_close archives meet.json (with inline decisions) into history.json.
