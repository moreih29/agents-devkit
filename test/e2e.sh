...
echo '[]' > "$E2E_HARNESS_STATE/agent-tracker.json"
...
check "SubagentStart (new entry added)" "agent-abc" "$(cat "$E2E_HARNESS_STATE/agent-tracker.json" 2>/dev/null || echo '')"

# (g) SubagentStart (duplicate agent_id) → resume_count++
echo '{"hook_event_name":"SubagentStart","agent_id":"agent-abc","agent_type":"claude-nexus:engineer"}' | node scripts/gate.cjs 2>/dev/null >/dev/null
check "SubagentStart (resume_count incremented)" "resume_count" "$(cat "$E2E_HARNESS_STATE/agent-tracker.json" 2>/dev/null || echo '')"
...
check "SubagentStop (files_touched injected)" "files_touched" "$(cat "$E2E_HARNESS_STATE/agent-tracker.json" 2>/dev/null || echo '')"
check "SubagentStop (tool-log file present)" "/tmp/x.ts" "$(cat "$E2E_HARNESS_STATE/agent-tracker.json" 2>/dev/null || echo '')"
